(() => {
  ...
  const LOGO = "https://cdn.prod.website-files.com/655a78032f46f2e55da300d7/69810a4d5326936131856311_iSchool%20Logo%20Small.svg";
  ...
  css.textContent = `
    ...
    #qa-login{position:fixed;inset:0;z-index:2147483647;display:grid;place-items:center;padding:24px;background:radial-gradient(circle at 10% 10%,rgba(5,111,236,.14),transparent 34%),radial-gradient(circle at 90% 85%,rgba(255,215,0,.14),transparent 32%),#f5f8fc;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    #qa-login-card{width:min(430px,100%);padding:34px;border:1px solid rgba(5,111,236,.15);border-radius:28px;background:rgba(255,255,255,.97);box-shadow:0 28px 80px rgba(11,35,74,.16)}
    #qa-login-card img{display:block;width:190px;height:auto;margin:0 auto 22px}
    ...
    .qa-field input{width:100%;height:48px;padding:0 14px;border:1px solid #dbe4ef;border-radius:14px;outline:none;background:#fff;color:#0f172a;transition:.2s ease}
    ...
    #qa-login-submit:hover{filter:brightness(.96)}#qa-login-error{min-height:20px;margin-top:12px;color:#AA1818;text-align:center;font-size:13px;font-weight:700}
    #qa-login-note{margin-top:18px;color:#85A5B9;text-align:center;font-size:11px}
    #qa-brand{display:flex;align-items:center;justify-content:center;margin:4px 10px 18px;padding:12px 10px;border-radius:16px;background:#fff;box-shadow:0 8px 24px rgba(11,35,74,.08)}
    #qa-brand img{display:block;width:145px;max-width:100%;height:auto}
    ...
    .text-blue-500,.text-blue-600,.text-blue-700,.text-blue-800,.text-sky-500,.text-sky-600{color:var(--brand-blue)!important}
    .bg-blue-500,.bg-blue-600,.bg-blue-700,.bg-sky-500,.bg-sky-600{background-color:var(--brand-blue)!important}
    .border-blue-400,.border-blue-500,.border-blue-600,.border-sky-400,.border-sky-500{border-color:var(--brand-blue)!important}
    @media(max-width:768px){#qa-login-card{padding:28px 22px;border-radius:22px}#qa-login-card img{width:165px}#qa-brand{display:none}#qa-logout{right:12px;bottom:12px}}
  `;
  ...
  function addBrand() {
    if (document.getElementById("qa-brand")) return;
    const aside = document.querySelector("aside");
    if (!aside) return;
    const holder = document.createElement("div");
    holder.id = "qa-brand";
    holder.innerHTML = `<img src="${LOGO}" alt="iSchool">`;
    aside.insertBefore(holder, aside.firstChild);
  }
