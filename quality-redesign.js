...
nav.forEach(x=>x.addEventListener('click',()=>openSection(x.dataset.section)));
document.querySelectorAll('[data-jump]').forEach(x=>x.addEventListener('click',()=>openSection(x.dataset.jump)));
...
let activeReviewMode='sampling';
if(reviewTabs.length>=2){
  reviewTabs[0].dataset.reviewMode='sampling';
  reviewTabs[0].textContent=reviewModes.sampling.label;
  reviewTabs[1].dataset.reviewMode='full';
  reviewTabs[1].textContent=reviewModes.full.label;
  const tabsBar=document.querySelector('#evaluation .tabs');
  const modeNotice=document.createElement('div');
  modeNotice.id='reviewModeNotice';
  modeNotice.className='notice';
  modeNotice.style.marginBottom='18px';
  tabsBar.insertAdjacentElement('afterend',modeNotice);
  const triggerCard=document.querySelector('#evaluation aside .card:last-child');
  const triggerTitle=triggerCard?.querySelector('h3');
  const triggerSub=triggerCard?.querySelector('.card-head p');
  const triggerNotice=triggerCard?.querySelector('.notice');
  function setReviewMode(mode){
    const config=reviewModes[mode]||reviewModes.sampling;
    activeReviewMode=mode;
    reviewTabs.forEach(tab=>tab.classList.toggle('active',tab.dataset.reviewMode===mode));
    modeNotice.innerHTML=`<b>${config.label}</b><br>${config.notice}`;
    if(document.getElementById('evaluation').classList.contains('active'))document.getElementById('pageSub').textContent=config.subtitle;
    if(triggerTitle)triggerTitle.textContent=mode==='sampling'?'Full Review trigger':'Full Review mode';
    if(triggerSub)triggerSub.textContent=mode==='sampling'?'Use when sampling is insufficient.':'Complete-session evidence review is active.';
    if(triggerNotice)triggerNotice.textContent=mode==='sampling'?'Possible flag, missing evidence, unclear context or score-impacting uncertainty.':'Review the entire recording and capture complete timestamped evidence before finalising.';
  }
  reviewTabs.forEach(tab=>tab.addEventListener('click',()=>setReviewMode(tab.dataset.reviewMode)));
  const evaluationNav=document.querySelector('[data-section="evaluation"]');
  evaluationNav?.addEventListener('click',()=>setReviewMode(activeReviewMode));
  document.querySelectorAll('[data-jump="evaluation"]').forEach(button=>button.addEventListener('click',()=>setReviewMode(activeReviewMode)));
...
const compliance=['Camera on','Clear sound','Professional dress','Correct logo/background','Correct language','No prohibited behaviour'];
document.getElementById('compliance').innerHTML=compliance.map((x,i)=>`<label class="check"><input type="checkbox" checked id="c${i}"> ${x}</label>`).join('');
function value(name){const x=document.querySelector(`input[name="${name}"]:checked`);return x?Number(x.value):0}
function avg(prefix){return experience.reduce((s,d)=>s+value(`${prefix}-${d.id}`),0)/experience.length}
...
['regularCore','regularExperience'].forEach(id=>document.getElementById(id).addEventListener('change',regularScore));
['s12Core','s12Experience','showcaseMetrics'].forEach(id=>document.getElementById(id).addEventListener('change',s12Score));
...
function gateScore(){const v=gate.map(id=>document.getElementById(id).value),badge=document.getElementById('gateBadge'),line=document.getElementById('gateLine');if(v.some(x=>x==='')){badge.textContent='Check required';badge.className='badge yellow';line.textContent='Pending'}else if(v.every(x=>x==='1')){badge.textContent='Eligible';badge.className='badge green';line.textContent='Eligible'}else{badge.textContent='Ineligible';badge.className='badge red';line.textContent='Select another case'}}
gate.forEach(id=>document.getElementById(id).addEventListener('change',gateScore));
...
document.querySelectorAll('[data-toast]').forEach(x=>x.addEventListener('click',()=>toast(x.dataset.toast)));
regularScore();
s12Score();
capacity();
